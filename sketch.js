...
function drawMirrorBars(spectrum, bass, mid) {
  ...
  for (let i = 0; i < numBars; i++) {
    ...
    fill(hue, sat, 90, 80);
    noStroke();
    rect(i * barWidth, height/2 - barHeight, barWidth - 2, barHeight, 3);

    // Bottom bars (mirror)
    fill(hue, sat, 70, 60);
    rect(i * barWidth, height/2, barWidth - 2, barHeight, 3);

    // Glow on peaks
    if (amp > 200) {
      fill(hue, 60, 100, 30);
      ellipse(i * barWidth + barWidth/2, height/2, barWidth * 2, barHeight * 0.3);
    }
  }
